...
<template>
  <div class="h-screen">
    ...
    <RouterLink :to="routes[index]" v-for="(name, index) in names" :key="index">
      <article class="bg-gray-700/30 p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
        ...
        <div class="mt-1 p-3">
            <div class="flex justify-between">
                <h2 class="text-slate-700 truncate">{{ name }}</h2>
            </div>
            <p class="mt-1 text-sm text-slate-400">Pesssoas pertencentes a uma determinada comunidade</p>
        </div>
      </article>
    </RouterLink>
    </div> 
